...
1. 일반 웹페이지에서 PixelScope 아이콘을 클릭합니다. 팝업 위쪽에는 **영역 측정**, **컬러 피커**, 객체·전체 페이지 캡처 실행 버튼이 먼저 표시됩니다. 측정·피커 오른쪽 정사각형 더보기 버튼을 누르면 모든 실행 버튼 아래의 설정 영역이 펼쳐집니다. 영역 측정 단위는 `px`, `rem`, `vw/vh` 중 선택할 수 있습니다.
...
6. **화면 캡처**의 **객체 캡처**는 강조된 DOM 요소를 클릭해 해당 경계 전체를 캡처합니다. **전체 페이지**는 문서 크기를 측정한 뒤 viewport 타일을 순서대로 캡처해 한 장의 PNG로 합성합니다.
7. 캡처가 끝나면 새 뷰어 탭에서 화면 맞춤, 10~400% 확대/축소, 이미지 클립보드 복사, PNG 저장을 사용할 수 있습니다. 원본 Blob은 뷰어 탭을 닫으면 제거되고 비정상 종료로 남은 데이터도 1시간 후 자동 정리됩니다.
...
src/viewer/*                    캡처 결과 복사·저장·확대/축소 뷰어
...
- 전체 페이지 캡처는 Chrome의 `captureVisibleTab()` 호출 제한에 맞춰 순차 실행되므로 긴 페이지일수록 시간이 걸립니다. 캡처 중 페이지 애니메이션이나 동영상이 재생되면 타일 사이 시점 차이가 생길 수 있습니다.
...
- 고정·sticky 요소는 페이지가 스크롤될 때 각 viewport 캡처에 반복해서 나타날 수 있습니다. iframe 내부 콘텐츠는 최상위 페이지에 보이는 결과만 포함되며 iframe 문서 단독 객체 선택은 지원하지 않습니다.
